...
module Spotlight
  ...
  class PagesController < Spotlight::ApplicationController
    ...
    def index
      ...
      exhibit = Spotlight::Exhibit.find_by(slug: params[:exhibit_id])
      @pages = exhibit.pages.for_default_locale.published.select { |p| can?(:read, p)}

      respond_to do |format|
        format.html
        format.json { render json: @pages.to_json(methods: [:thumbnail_image_url]) }
      end
    end
